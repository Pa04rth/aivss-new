...
# @mcp.tool
# def autoharden_agent(agent_path: str) -> dict:
#     """Analyze and harden an AI agent with security guardrails (MCP endpoint)."""
#     # Basic path validation (can be more robust)
#     if not os.path.exists(agent_path) or not agent_path.endswith('.py'):
#         return {"error": "Invalid or non-existent Python file path.", "success": False}

#     result = run_scan_on_file(agent_path)
...
#             send_results_to_webapp(result)
#         except Exception as e:
#             print(f"⚠️ Could not send results to web app from MCP: {e}")
...
# Load environment variables for the scanner logic
load_dotenv()

# --- All core logic now lives in this file ---
...
class StaticRiskDetector(ast.NodeVisitor):
    ...
    def visit_Call(self, node: ast.Call):
        ...
        if func_name:
            self._check_risk_patterns(func_name, line_no)
        self.generic_visit(node)
    def _get_function_name(self, node: ast.Call):
        if isinstance(node.func, ast.Name):
            return node.func.id
        elif isinstance(node.func, ast.Attribute):
            if isinstance(node.func.value, ast.Name):
    ...
    def _check_risk_patterns(self, func_name: str, line_no: int):
        if func_name in self.CRITICAL_CALLS:
            self.risks.append(Risk(file_path=self.file_path, line_number=line_no, risk_type='critical_security', severity='critical', message=f'🚨 Critical: {func_name}() detected', suggestion=self.CRITICAL_CALLS[func_name], source='static'))
        elif any(p in func_name for p in self.MEDIUM_RISK_PATTERNS):
            pattern = next(p for p in self.MEDIUM_RISK_PATTERNS if p in func_name)
            self.risks.append(Risk(file_path=self.file_path, line_number=line_no, risk_type='medium_security', severity='medium', message=f'⚠️ Medium risk: {pattern} usage detected', suggestion=self.MEDIUM_RISK_PATTERNS[pattern], source='static'))
        elif any(p in func_name for p in self.LOW_RISK_PATTERNS):
            pattern = next(p for p in self.LOW_RISK_PATTERNS if p in func_name)
            self.risks.append(Risk(file_path=self.file_path, line_number=line_no, risk_type='low_security', severity='low', message=f'ℹ️ Low risk: {pattern} usage detected', suggestion=self.LOW_RISK_PATTERNS[pattern], source='static'))

def analyze_and_comment_codebase(root_dir: str, gemini_response: dict = None):
    # This is a placeholder for the full function logic
    print(f"Annotating files in {root_dir}...")
    pass

def inject_guardrails_yaml(yaml_path, guardrails):
    # This is a placeholder for the full function logic
    print(f"Injecting guardrails into {yaml_path}...")
    pass

def retrieve_context(query, k=3):
    # Placeholder for RAG logic
    print(f"Retrieving context for query: {query}")
    return "Dummy context about security."

def extract_metadata_from_file(filepath):
    with open(filepath, "r", encoding="utf-8") as f:
        code = f.read()
    agent_card = {"name": os.path.basename(filepath).replace('.py', ''), "description": f"AI agent from {filepath}", "file_path": filepath}
...
def get_guardrails(agent_data, tool_data, routing_logic, retrieved_context=""):
    ...
    # Simulating the Gemini call to avoid actual API costs during debugging
    print("Simulating Gemini API call...")
    dummy_response_text = json.dumps({
        "constraints": [{"description": "Simulated constraint", "severity": "low"}],
        "risks": [{"description": "Simulated risk from AI", "severity": "medium", "impact": "Simulated impact"}],
        "hardened_code": ["# Simulated hardened code suggestion"]
    })
    return parse_guardrail_response(dummy_response_text)
...
def run_scan_on_file(file_path: str) -> dict:
    try:
        ...
        retrieved_context = retrieve_context("prompt injection, tool misuse")
        agent_card, tool_schema, routing_logic = extract_metadata_from_file(file_path)
        
        print("🧠 Generating guardrail suggestions...")
        parsed = get_guardrails(json.dumps(agent_card), json.dumps(tool_schema), routing_logic, retrieved_context)
        
        if not parsed:
            return {"error": "Failed to generate guardrails from AI", "success": False}
        
        detector = StaticRiskDetector(file_path)
        with open(file_path, 'r', encoding='utf-8') as f:
            tree = ast.parse(f.read())
        detector.visit(tree)
        static_risks = [vars(risk) for risk in detector.risks]

        # Combine AI risks and static risks
        all_risks = parsed.get("risks", []) + static_risks
